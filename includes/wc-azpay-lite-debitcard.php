...
/**
 * WC AZPay Lite Debitcard
 * Payment Gateway
 *
 * Provides a debitcard payment method,
 * integrated with AZPay Gateway.
 *
 * @class 		WC_AZPay_Lite_Debitcard
 * @extends		WC_Payment_Gateway
 */

class WC_AZPay_Lite_Debitcard extends WC_Payment_Gateway {

	public $debitcard_config = null;

	public function __construct() {

		$this->id           = 'azpay_lite_debitcard';
		$this->icon         = null;
		$this->has_fields   = true;
		$this->method_title = 'AZPay - Débito';
		$this->title = 'Cartão de Débito';
		...
		if (is_admin()) {
	        ...
	        if (version_compare( WOOCOMMERCE_VERSION, '2.0.0', '>=')) {
	        	...
	        	add_action('woocommerce_update_options_payment_gateways_' . $this->id, array(&$this, 'save_debitcard_config'));
            } else {
                add_action('woocommerce_update_options_payment_gateways', array(&$this, 'process_admin_options'));
	        	add_action('woocommerce_update_options_payment_gateways', array(&$this, 'save_debitcard_config'));
            }
	    }
	}
	...
	public function init_form_fields() {
		...
		$fields = array(
			...
			'azpay_lite_title' => array(
				'title' => 'Woocommerce AZPay Lite - Cartão de Débito',
				'type'  => 'title'
			),
			'enabled' => array(
			...
			'debitcard_form_description' => array(
				'title'       => 'Descrição',
				'type'        => 'textarea',
	...
	public function process_payment($order_id) {
		...
		try {
			...
			$payment_method_config = get_option( 'woocommerce_azpay_lite_debitcard_settings' );
	...
	public function currency_not_supported_message() {
		echo '<div class="error"><p><strong>AZPay Lite - Cartão de Débito</strong>: Moeda não aceita</p></div>';
	}
	 ...
	 * @return [type] [description]
	 */
	public function save_Debitcard_config() {

		if (isset($_POST['woocommerce_azpay_lite_debitcard']))
			update_option('woocommerce_azpay_lite_debitcard', json_encode($this->filterData($_POST['woocommerce_azpay_lite_debitcard'])));
	}
	...
	public function payment_fields() {
		...
		$html_path = apply_filters('wc_azpay_debitcard_form', plugin_dir_path(__FILE__) . 'views/html-form-debitcard.php');
